...
class ModelTest:
    def __init__(self):
        ...
        self.feature = self.config["ml_params"]["feature"]
        ...
        self.best_model_params = self.config["ml_params"]["best_model_params"]
        self.X_test_preprocessed = self.config["preprocessed_data"]["X_test"]
        self.y_test = self.config["preprocessed_data"]["y_test"]
    ...
    def get_predictions(objects, X_test, y_test):
        prediction = objects["model"].predict(X_test)
    ...
    def test(self):
        try:
            ...
            X_test = self.os.path.join(self.preprocessed_dataset_path, self.X_test_preprocessed)
            y_test = self.os.path.join(self.preprocessed_dataset_path, self.y_test)
